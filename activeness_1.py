...
def actvness(region_num):
    ...
    """[df_veh['BESTMILE']<=0]"""
    # vehicle side id
    df_veh = df_veh.loc[df_veh['BESTMILE']>100]
    ...
    df_veh = df_veh.loc[df_veh['VEHTYPE'].isin([1,2,3,4])]

    # total vehicle count
    lst = []
    for jj in range(2) :
        j = [TRAVDAY_g1,TRAVDAY_g2][jj]
        lst.append(df_veh[df_veh['TRAVDAY'].isin(j)].shape[0])
    # total vehicle count
    lst = []
    ...
    for jj in range(2) :
        j = TRAVDAY_g[jj]
        df_1 = df_veh[df_veh['TRAVDAY'].isin(j)]
    ...
    for hi in hhlst:
        ...
        for vi in vehid:
            ...
            hh_veh_lst[1].append(df3['TRAVDAY'][df3['TRAVDAY'].index[0]])
    ...
    for jj in range(2) :
        j = [TRAVDAY_g1,TRAVDAY_g2][jj]
        lst_1.append(df4[df4[1].isin(j)].shape[0])
    df0 = pd.DataFrame([lst,lst_1,[lst_1[i]/lst[i] for i in range(len(lst))]])
    
    df0.to_csv(r'C:\Users\jiahuic\Dropbox (University of Michigan)\Ford_CC\Driving_profile_simu\activeness_'+region+'_1.csv')
