...
# output = r'G:\Dropbox (University of Michigan)\Ford_CC\\'
output = r'C:\Users\jiahuic\Dropbox (University of Michigan)\Ford_CC\\'
...
for bati in range(4):
    for chargei in range(2,3):
        for county_num in range(432):
            df = pd.read_csv(output + r'Server_168h\Deginterm_1\\'
                                    +str(county_num)+'_80Y'+caplst[capi]+'_'+chargelst[chargei]+'_'+batmodlst[bati]+'_cons.csv')
...
pd.DataFrame(lst0).to_csv(output + r'Server_168h\Deginterm_2\\degmid_'+caplst[capi]+'_'+caplst[capi]+'_cons.csv')
