...
# num0,num1,year,cc_ip,urb,car,lcc=0,1,2024,0,0,0,0
    ...
    for county_numi in range(len(county_num_lst)):
        ...
        for yr in [2024,2030,2040]:
            ...
            yr = str(yr)
            for building in ['']:
                fnames = os.listdir(outputdir+r'\\Result_db\\Result'+cc_hand+r'\\Results_'+'80_82_1'+r'\\'+str(county_num)+building)
                ...
                if len(fnames) > 0:
                    ...
                    for fnamei in range(len(fnames)):
                        fname = fnames[fnamei]
                        res = pd.read_csv(outputdir+r'\\Result_db\\Result'+cc_hand+r'\\Results_'+'80_82_1'+r'\\'+str(county_num)+building+'\\'+fname)
