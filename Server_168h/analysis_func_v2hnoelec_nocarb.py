...
# num0,num1,year,cc_ip,urb,car,lcc=0,1,2024,0,0,0,0
    ...
    # ctydf = pd.read_csv(r'G:\\Dropbox (University of Michigan)\Ford_CC\2020_Gaz_counties_national_0728_tempreg.csv')
    ctydf.set_index(keys=ctydf['county_num'],inplace=True)
    # number of columns:
    lenc = 3
    # UC urb car lcc
    lst = [[] for i in range(lenc)]
    cc_hand = ['s','_cc','_v2h_no'][cc_ip]
    ...
    for county_numi in range(len(county_num_lst)):
        ...
        for building in ['_0_0']:
            fnames = os.listdir(outputdir+r'\\Result'+cc_hand+r'\\Results_'+year+r'_nocarb\\'+str(county_num)+building)
            ...
            if len(fnames) > 0:
                ...
                for fname in fnames:
                    res = pd.read_csv(outputdir+r'\\Result'+cc_hand+r'\\Results_'+year+r'_nocarb\\'+str(county_num)+building+'\\'+fname)
    ...
    df2.to_csv(outputdir+r'\\Server_168h\csv_trb\\'
                +str(num0)+'_'+str(num1)+'_'+str(year)+'_'
                +str(cc_ip)+'_'+str(urb)+'_'+str(car)+'_'+str(lcc)+'_noelec_nocarb.csv')
