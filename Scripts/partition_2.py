...
def funcs(region_num):
    ...
    df_trip['STRTTIME'] = df_trip.apply(lambda row: row['STRTTIME']+2400 if row['STRTTIME']<=400 else row['STRTTIME'], axis=1)
    df_trip['ENDTIME'] = df_trip.apply(lambda row: row['ENDTIME']+2400 if row['ENDTIME']<=400 else row['ENDTIME'], axis=1)
    df_trip['ENDTIME'] = df_trip.apply(lambda row: row['STRTTIME']+1 if row['ENDTIME']<=row['STRTTIME'] else row['ENDTIME'], axis=1)
    ...
    df_trip['TRPMILES'] = df_trip.apply(lambda row: 
                                    (int(row['TRVLCMIN'])/60)
                                    *60
                                    if row['Speed']<0 or row['Speed']>150
                                        else row['TRPMILES'],axis=1
    )
                ...
                for weekend_i in range(2):
                    ...
                    for hh in [*set(df_indi_trp['HOUSEID'].to_list())]:
                        for vv in [*set(df_indi_trp[df_indi_trp['HOUSEID']==hh]['VEHID'].to_list())]:
                            ...
                            df2 = df1.loc[df1['STRTTIME']==min(df1['STRTTIME'])]
                            df2 = df2.loc[df2['ENDTIME']==max(df2['ENDTIME'])]
                            df2 = df2.loc[df2.index[0],:]
                            df3 = pd.DataFrame(df2).T
                            df1 = (df1.loc[df1['STRTTIME']>=df2['ENDTIME']]) # drip the recorded trip 
                            while len(np.unique(df1['STRTTIME'])) >1:
                                df2 = df1.loc[df1['STRTTIME']==min(df1['STRTTIME'])]
                                df2 = df2.loc[df2['ENDTIME']==max(df2['ENDTIME'])]
                                df2 = df2.loc[df2.index[np.where(df2['TRPMILES'].to_numpy()==max(df2['TRPMILES']))[0]][0],:]
                                df3 = pd.concat([df3,pd.DataFrame(df2).T])
                                endtime = df2['ENDTIME']                            
                                df1 = (df1.loc[df1['STRTTIME']>endtime]) # next trip
                            if df1.ndim >=1 and df1.shape[0]>=1 :
                                df2 = pd.DataFrame(df1.loc[df1.index[0],:]).T
                                df3 = pd.concat([df3,df2])
                            try:os.mkdir(basedir+r'\Raw\trip_regions\\'+region+r'\\')
                            except:pass
                            df3.to_csv(basedir+r'\Raw\trip_regions\\'+region+r'\\'+str(hh)+'_' + str(vv) +'.csv')
