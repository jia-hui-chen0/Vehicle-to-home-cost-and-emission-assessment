...
def Parkingrecog(num0, num1):
    ...
    # basedir = r'G:\Dropbox (University of Michigan)\Ford_CC'
    basedir = r'C:\Users\jiahuic\Dropbox (University of Michigan)\Ford_CC\\'
    ...
    for county_num in range(num0,num1):
        ...
        flst = [i for i in flst if '0_0_0' in i]
        for fname in flst:
            cari = int(fname[2])
            urbi = int(fname[0])
            ...
            try:
                os.mkdir(outdir + r"\\Energy consumption_adjusted_1_1_Y82_db\\"+str(county_num))
            except:
                pass
            try:
                os.mkdir(outdir + r"\\Energy consumption_adjusted_1_1_Y82_db\\"+str(county_num)+"\\SC_opp_"+trucknm)
                os.mkdir(outdir + r"\\Energy consumption_adjusted_1_1_Y82_db\\"+str(county_num)+"\\FC_opp_"+trucknm)
            except:
                pass

            df2.to_csv(outdir + r"\\Energy consumption_adjusted_1_1_Y82_db\\"+str(county_num)+"\\SC_opp_"+trucknm+'\\'+fname)
            lst_fc = [i+1 for i in range(8760) if i+1 not in lst_chg]
            df3 = pd.DataFrame(lst_fc,columns=['Hours'])
            df3.to_csv(outdir + r"\\Energy consumption_adjusted_1_1_Y82_db\\"+str(county_num)+"\\FC_opp_"+trucknm+'\\'+fname)
