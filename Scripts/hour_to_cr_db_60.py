...
def CC(num0,num1,years):
    ...
    for county_num in range(num0,num1):
        try:
            os.mkdir(outputdir+r'\\Energy consumption_adjusted_1_2_Y60_db\\')
        except:pass
        try:
            os.mkdir(outputdir+r'\\Energy consumption_adjusted_1_2_Y60_db\\'+str(county_num))
        except:pass
        try:
            os.mkdir(outputdir+r'\\Energy consumption_adjusted_1_2_Y60_db\\'+str(county_num)+'\\FC_opp_SUV\\')
        except:pass
        try:
            os.mkdir(outputdir+r'\\Energy consumption_adjusted_1_2_Y60_db\\'+str(county_num)+'\\SC_opp_SUV\\')
        except:pass
        try:
            os.mkdir(outputdir+r'\\Energy consumption_adjusted_1_2_Y60_db\\'+str(county_num)+'\\FC_opp_Truck\\')
        except:pass
        try:
        ...
        try:
            os.mkdir(r'E:\Dropbox (University of Michigan)\Ford_CC\Energy consumption_adjusted_1_2_Y60_db\\'+str(county_num))
            os.mkdir(r'E:\Dropbox (University of Michigan)\Ford_CC\Energy consumption_adjusted_1_2_Y60_db\\'+str(county_num)+'\\'+'FC_opp_SUV')
            os.mkdir(r'E:\Dropbox (University of Michigan)\Ford_CC\Energy consumption_adjusted_1_2_Y60_db\\'+str(county_num)+'\\'+'SC_opp_SUV')
        except:pass
        for fname in fnames:
            # veh_fname = fname[:-10]
            cari = int(fname[2])
            ...
            for year in [years]:
                ...
                pd.DataFrame(fc_crarr,columns=['fastCR']).to_csv(outputdir + r"\\Energy consumption_adjusted_1_2_Y60_db\\"+str(county_num)+"\\FC_opp"+tcnm+'\\'+fname)
                pd.DataFrame(sc_crarr,columns=['slowCR']).to_csv(outputdir + r"\\Energy consumption_adjusted_1_2_Y60_db\\"+str(county_num)+"\\SC_opp"+tcnm+'\\'+fname)
                    
                pd.DataFrame(soc_drop,columns=['SOC']).to_csv(outputdir + r"\\Energy consumption_adjusted_1_2_Y60_db\\"+str(county_num)+"\\"+fname)
