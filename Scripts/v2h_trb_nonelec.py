    ...
    for county_num in range(num0,num1):
        ...
        for fname_i in range(len(fnames)):
            ...
            for year in [years]:
                ...
                for i1 in range(1):
                    for i2 in range(1):
                        sd_drarr = pd.read_csv(outputdir +r'\\ResStock\\dischargecons_oelec_Y82\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+'\\'+fname_evhh)['CR'].to_numpy()
                        ...
                        # except: pass
                        try: 
                            os.mkdir(outputdir+r'\\Result_v2h\\Results_noelec_80_Y82_'+str(year)+r'\\')
                        except: pass
                        try: os.mkdir(outputdir+r'\\Result_v2h\\Results_noelec_80_Y82_'+str(year)+r'\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+r'\\')
                        except: pass
                        df_res.T.to_csv(outputdir+r'\\Result_v2h\\Results_noelec_80_Y82_'+str(year)+r'\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+r'\\'+fname_evhh)
...
if __name__ == '__main__':
    ...
    for yea in [2024,2030,2040]:
        for i in range(a):
            if i != a-1:
                p = Process(target=CC, args=([i*b+1,i*b+b,yea]))
            else:
                p = Process(target=CC,args=([i*b+1,432,yea]))
            p.start()
            procs.append(p)
        for p in procs:
            p.join() 
