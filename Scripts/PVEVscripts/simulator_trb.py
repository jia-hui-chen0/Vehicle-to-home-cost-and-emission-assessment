...
def simulator(region_num,start):
 ...
 0.8110228934530891]
    ...
    basedir2 = r'C:\Users\jiahuic\Dropbox (University of Michigan)\PVEV\\FORD_CC\\'
    veh = 0
    vehnm = ['','_truck']
    simu = 1
    ...
    try:os.mkdir(basedir2+r'\\Driving_profile_simu_trb\\')
    except: pass
    try:os.mkdir(basedir2+r'\\Driving_profile_simu_trb\\'+region+'\\')
    except: pass
    ...
    for cari in range(2):
        for weekend_i in range(2):
            df_trip_1 = df_trip[df_trip['VEHTYPE'].isin(car_group[cari])][df_trip['TRAVDAY'].isin(TRAVDAY_g[weekend_i])]
            ...
            vehid_region[cari].append(vehid_org_dest)
    df_act = pd.read_csv(basedir+r'\\Driving_profile_simu_trb\activeness_trb_'+region+'.csv')
    ...
    for cari in range(2):
        ...
        for dayi in range(365):
            ...
            if random.uniform(0,1) <= df_act.transpose()[1:][2][weekend_i*2+cari]/coe:
                vehid = []
                for grp_i in range(3):
                    vehid = vehid+ vehid_region[cari][weekend_i][start_loc][grp_i]
                if len(vehid) ==0:
                    lst_0tp.append([cari,weekend_i,start_loc])
                    start_loc = 0
                else: pass
                vehid = []
                for grp_i in range(3):
                ...
                try:
                    ...
                    print([cari,weekend_i,start_loc,region_num])
                    print(len(vehid))
                vehid_sam = vehid[a]
        ...
        dflst[cari].append(df)

    ## store the samples

    for cari in range(2):
        for veh_i in range(len(dflst[cari])):
            df_dflst = pd.DataFrame(dflst[cari][veh_i])
            df_dflst = df_dflst[["TRPMILES", "TRVLCMIN","STRTTIME",
                                    'ENDTIME','WHYFROM','WHYTRP1S','DAY','VMT_MILE'
                                    ]]
            id = str(0) + '_' + str(cari) + '_' + str(0) + '_' +str(veh_i+start)
            try:os.mkdir(basedir2+r'\\Driving_profile_simu_trb\\'+region+'\\')
            except:pass
            df_dflst.to_csv(basedir2+r'\\Driving_profile_simu_trb\\'+region+'\\'+ id+ '.csv')
