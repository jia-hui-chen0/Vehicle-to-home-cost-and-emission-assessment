...
def actvness(region_num):
    ...
    for jj in range(2) :
        for kk in range(2) :
            ...
            lst.append(df_veh[df_veh['VEHTYPE'].isin(k)][df_veh['TRAVDAY'].isin(j)].shape[0])
    # total vehicle count
    lst = []
    ...
    for jj in range(2) :
        for kk in range(2):
            k = [car_group_1, car_group_2][kk]
            j = TRAVDAY_g[jj]
            df_1 = df_veh[df_veh['VEHTYPE'].isin(k)][df_veh['TRAVDAY'].isin(j)]
    ...
    for jj in range(2) :
        for kk in range(2) :
            j = [TRAVDAY_g1,TRAVDAY_g2][jj]
            k = [car_group_1, car_group_2][kk]
            lst_1.append(df4[df4[3].isin(k)][df4[2].isin(j)].shape[0])
    if region_num == 5:
        df0 = pd.DataFrame([lst,lst_1,[lst_1[i]/lst[i] for i in range(len(lst))]])
    else:
        df0 = pd.DataFrame([lst,lst_1,[lst_1[i]/lst[i] for i in range(len(lst))]])

    df0.to_csv(r'C:\Users\jiahuic\Dropbox (University of Michigan)\Ford_CC\Driving_profile_simu_trb\activeness_trb_'+region+'.csv')
