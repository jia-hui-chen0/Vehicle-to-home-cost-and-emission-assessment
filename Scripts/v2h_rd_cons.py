...
execute_unload "results.gdx" sc fc vSOC sd; '''
    ...
    foldername = os.listdir(r'C:\Users\jiahuic\Dropbox (University of Michigan)\Ford_CC\Result_v2h\Results_80_Y82_2024_cons')
    ...
    for county_num in range(num0,num1):
        ...
        if len(os.listdir(root_loc + r'\\Result_v2h\Results_80_Y82_'+str(years)+'_cons\\' + str(county_num)+'_0_0')) != 15:
            fnames = [i for i in fnames if i not in os.listdir(root_loc + r'\\Result_v2h\Results_80_Y82_'+str(years)+'_cons\\' + str(county_num)+'_0_0')]
            for fname_i in range(len(fnames)):
                ...
                for year in [years]:
                    ...
                    for i1 in range(1):
                        for i2 in range(1):
                            sd_drarr_0 = pd.read_csv(outputdir +r'\\ResStock\\dischargecons_Y82\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+'\\'+fname_evhh)['CR'].to_numpy()
                            hours_per_day = 24
                            ...
                            try: 
                                # os.mkdir(outputdir+r'\\Results_opt_3\\')
                                os.mkdir(outputdir+r'\\Result_v2h\\Results_80_Y82_'+str(year)+r'_cons\\')
                            except: pass
                            try: os.mkdir(outputdir+r'\\Result_v2h\\Results_80_Y82_'+str(year)+r'_cons\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+r'\\')
                            except: pass
                            df_res.T.to_csv(outputdir+r'\\Result_v2h\\Results_80_Y82_'+str(year)+r'_cons\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+r'\\'+fname_evhh)
