...
def UC(num0,num1):
    ...
    for county_num in range(num0,num1):
        try:os.mkdir(outputdir+r'\\Result_db\\Results\Results_80_60_1\\')
        except:pass
        try:os.mkdir(outputdir+r'\\Result_db\\Results\Results_80_60_1\\'+str(county_num)+r'\\')
        except:pass
        for fname in fnames:
            cari = int(fname[2])
            urbi = int(fname[0])
            ...
            if cari==0 and urbi ==0 and lcci==0:
                ...
                fc_hr = pd.read_csv(outputdir+r'\\Energy consumption_adjusted_1_2_Y60_db\\'+str(county_num)+r'\\'+r'FC_opp'+tcnm+r'\\'+fname)
                ...
                if results[2][1]!=82*0.3:
                    ...
                    try:
                        os.mkdir(outputdir+r'\\Result_db\\Results\\Results_80_60_1\\'+str(county_num)+r'\\')
                    except:
                        pass
                    df_res.T.to_csv(outputdir+r'\\Result_db\\Results\\Results_80_60_1\\'+str(county_num)+r'\\'+fname)
