...
execute_unload "results.gdx" sc fc vSOC; '''
    ...
    for year in years:
        try: os.mkdir(outputdir+r'\\Result_db\\Result_cc\\')
        except:pass
        try:
            
            os.mkdir(outputdir+r'\\Result_db\\Result_cc\\Results_Y82_'+str(year))
            
        except:pass
        for county_num in range(num0,num1):
            try: os.mkdir(outputdir+r'\\Result_db\\Result_cc\\Results_Y82_'+str(year)+'\\'+str(county_num)+'_'+str(0)+'_'+str(0)+r'\\')
            except:pass
    for year in years:
        ...
        for county_num in range(num0,num1):
            ...
            for fname_i in range(len(fnames)):
                ...
                df_res.T.to_csv(outputdir+r'\\Result_db\\Result_cc\\Results_Y82_'+str(year)+r'\\'+str(county_num)+'_'+str(0)+'_'+str(0)+r'\\'+fname)
...
if __name__ == '__main__':
    ...
    # b=432//a
    # for i in range(432):
    #     p = Process(target=CC, args=([i,i+1,[2024,2030,2040]]))
    #     p.start()
    #     procs.append(p)
    # for p in procs:
    #     p.join() # this blocks until the process terminates