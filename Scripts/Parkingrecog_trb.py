...
def Parkingrecog(num0, num1):
    ...
    for county_num in range(num0,num1):
        ...
        flst = os.listdir(outdir+r'\\Energy consumption_adjusted_1_1_Y82\\'+str(county_num)+'\\')
        flst = [i for i in flst if '.csv' in i][5:20]
        for fname in flst:
            cari = int(fname[2])
            urbi = int(fname[0])
            trucknm = ['SUV','Truck'][cari]

            df0 = pd.read_csv(outdir+r'\\Energy consumption_adjusted_1_1_Y82\\'+str(county_num)+'\\'+fname)
            ...
            try:
                os.mkdir(outdir + r"\\Energy consumption_adjusted_1_2_Y82\\"+str(county_num))
            except:
                pass
            try:
                os.mkdir(outdir + r"\\Energy consumption_adjusted_1_2_Y82\\"+str(county_num)+"\\SC_opp_"+trucknm)
                os.mkdir(outdir + r"\\Energy consumption_adjusted_1_2_Y82\\"+str(county_num)+"\\FC_opp_"+trucknm)
            except:
                pass

            df2.to_csv(outdir + r"\\Energy consumption_adjusted_1_2_Y82\\"+str(county_num)+"\\SC_opp_"+trucknm+'\\'+fname)
            lst_fc = [i+1 for i in range(8760) if i+1 not in lst_chg]
            df3 = pd.DataFrame(lst_fc,columns=['Hours'])
            df3.to_csv(outdir + r"\\Energy consumption_adjusted_1_2_Y82\\"+str(county_num)+"\\FC_opp_"+trucknm+'\\'+fname)
