...
def UC(num0,num1):
    ...
    SOC = [62,153]
    fnames = os.listdir(outputdir+r'\\Energy consumption_adjusted_1_1\\0')
    fnames = [i for i in fnames if r'.csv' in i]
    faillst = []
    for county_num in range(num0,num1):
        try:os.mkdir(outputdir+r'\\Results\Results_2026_62\\'+str(county_num)+r'\\')
        except:pass
        for fname in fnames:
            cari = int(fname[2])
            urbi = int(fname[0])
            ...
            if cari==0 and urbi ==0 and lcci==0:
                ...
                df0 = pd.read_csv(outputdir+r'\\Energy consumption_adjusted_1_1\\'+str(county_num)+r'\\'+fname)
                ...
                for i in range(trpnum):
                    ...
                    duration = endhr - strhr
                    mile = df0['VMT_MILE'][i]
                    if acc_soc_drop >= 0.4 * vCap and duration >= thresh and loc in [0,1] :
                ...
                fc_hr = pd.read_csv(outputdir+r'\\Energy consumption_adjusted_1_2\\'+str(county_num)+r'\\'+r'FC_opp'+tcnm+r'\\'+fname)
                ...
                fc_ecarr = np.arange(8760) + 10000.5
                fc_ec = fc_ecarr
                fastEC = dict(zip(hr,fc_ec))
                #   hourly SOC drop
                soc_drop_df = pd.read_csv(outputdir+r'\\Energy consumption_adjusted_1_2\\'+str(county_num)+r'\\'+fname)
                ...
                try:os.mkdir(outputdir+r'\\Results\\Results_2026_62\\')
                except:pass
                try:
                    os.mkdir(outputdir+r'\\Results\\Results_2026_62\\'+str(county_num)+r'\\')
                except:
                    pass
                df_res.T.to_csv(outputdir+r'\\Results\\Results_2026_62\\'+str(county_num)+r'\\'+fname)
