...
def ICEV(num0, num1):
    ...
    for county_num in range(num0,num1):
        ...
        nlst = [i for i in os.listdir(r'C:\jiahuic\Dropbox (University of Michigan)\Ford_CC\Energy consumption_adjusted\0') 
            if os.path.isfile(r'C:\jiahuic\Dropbox (University of Michigan)\Ford_CC\Energy consumption_adjusted\0\\'+i)]
        nlst = [i[:-10]+'.csv' for i in nlst]
        # MPGe fuel economy from fuel economy
        cty_lst = [1/32.7,1/25.4]
        hwy_lst = [1/39.9,1/31.3]
        ...
        for fname in nlst:
            ...
            if urban == 0:
                ...
                # hwyOR = 33.70/hwy_lst[cari]
                cityNT = 1/(0.00187+(1.134/(hwy_lst[cari])) )
