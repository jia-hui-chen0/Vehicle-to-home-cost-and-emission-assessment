...
$gdxin
  ...
  execute_unload "results.gdx" sc fc vSOC; '''
    root_loc = r'C:\jiahuic\Dropbox (University of Michigan)\Ford_CC\\'
    outputdir = root_loc
    ...
    try:
        os.mkdir(outputdir+r'\\Results_opt\\Results_'+str(year))
        try: os.mkdir(outputdir+r'\\Results_opt\\Results_'+str(year)+'\\'+str(county_num)+r'\\')
        except:pass
    except:pass
    ...
    for county_num in range(num0,num1):
        ...
        for fname in fnames:
            ...
            if cari==0 and urbi ==0 and lcci==0:
                ...
                for year in [years]:
                    ws = GamsWorkspace(system_directory=r'C:\GAMS\45')
                    ...
                    try: os.mkdir(outputdir+r'\\Results_opt\\Results_'+str(year)+r'\\')
                    except: pass
                    try: os.mkdir(outputdir+r'\\Results_opt\\Results_'+str(year)+r'\\'+str(county_num)+r'\\')
                    except: pass
                    df_res.T.to_csv(outputdir+r'\\Results_opt\\Results_'+str(year)+r'\\'+str(county_num)+r'\\'+fname)
