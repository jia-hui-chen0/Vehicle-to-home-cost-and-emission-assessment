...
def soc_drop(num0, num1):
    ...
    for county_num in range(num0,num1): # county num is now temp reg
        ...
        def tempimpact(tem1):
            ...
            return coeT1
        for fname in [i for i in os.listdir(driving_profile_foldername) if i[0:6]=='0_0_0_']:
        # for fname in [j for j in os.listdir(driving_profile_foldername) if j[0]==j[2]== '1' and j[4]=='0']:
            ...
            if urban == 0:
                ...
                comb = (hwyNT*0.35+cityNT*0.65)*248/272
            elif urban ==1:
                hwyOR = 33.70/hwy_lst[cari]
                hwyNT = 33.70/hwy_lst[cari]*0.7 + 0.87*(hwyOR - 33.70/hwy_lst[cari]*0.7)
                cityOR = 33.70/cty_lst[cari]
            ...
            try: os.mkdir(outdir+r'\\Energy consumption_adjusted_1_1\\')
            except: pass                
            try: os.mkdir(outdir+r'\\Energy consumption_adjusted_1_1\\'+str(county_num))
            except: pass
            outputdir = outdir+r'\\Energy consumption_adjusted_1_1\\'+str(county_num)+'\\'
