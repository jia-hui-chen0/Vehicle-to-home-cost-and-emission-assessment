...
Variables
    ...
    sd.lo(h) = 0;
    sd.up(h) = sd_dr(h);
    vSOC.lo(h) = 0.3 *28*3.3;
    vSOC.up(h) = 1 * 28*3.3;
    vSOC.fx(h) $ (ord(h) eq 1) = vIni;
    vSOC.fx(h) $ (ord(h) eq 240) = 28*3.3;
Equations
...
execute_unload "results.gdx" sc fc vSOC sd; '''
    ...
    for county_num in range(358,359):
        ...
        for fname_i in range(len(fnames)):
            ...
            for year in [years]:
                ...
                # cost = cost + emission*0.051+0.075 # 
                ...
                for i1 in range(2):
                    for i2 in range(2):
                        fnames_1 = os.listdir(outputdir+r'\\dischargecons\\'+str(county_num)+'_'+str(i1)+'_'+str(i2))
                        fname_evhhs = [i for i in fnames_1 if '50'+ '_0_0_0_' in i]
                        
                        for fname_evhh in fname_evhhs:
                            sd_drarr = pd.read_csv(outputdir +r'\\dischargecons\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+'\\'+fname_evhh)['CR'].to_numpy()
                            ...
                            for cyclei in range(ttncycle):
                                ...
                                if cyclei!=ttncycle-1:
                                    hr1 = hr[cyclei*24:cyclei*24+48]
                                else:
                                    hr1 = hr[cyclei*24:8760]
                                ...
                                for symbi in range(4):
                                    ...
                                    if cyclei!=ttncycle-1:
                                        totalresults[symbi] = np.concatenate((np.array(totalresults[symbi]),np.array(result[0:24])))
                                        
                                    else:
                                        totalresults[symbi] = np.concatenate((np.array(totalresults[symbi]),np.array(result)))
                                        
                                if cyclei!=ttncycle-1: vIni_val = results[2][24]
                                else:pass
                                print(cyclei)
                            ...
                            #     os.mkdir(outputdir+r'\\Result_v2h\\Results_'+str(year)+r'\\')
                            # except: pass
                            # try: os.mkdir(outputdir+r'\\Result_v2h\\Results_'+str(year)+r'\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+r'\\')
                            # except: pass
                            # df_res.T.to_csv(outputdir+r'\\Result_v2h\\Results_'+str(year)+r'\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+r'\\'+fname_evhh)
                            df_res.T.to_csv(outputdir+r'\\Case studies\\' +str(i1)+'_'+str(i2)+'_'+ 'Chicago.csv')
