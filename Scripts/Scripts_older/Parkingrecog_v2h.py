...
def Parkingrecog(num0, num1):
    ...
    for county_num in range(num0,num1):
        ...
        flst = os.listdir(outdir+r'\\Energy consumption_adjusted\\'+str(county_num)+'\\')
        ...
        for fname in flst:
            ...
            df0 = pd.read_csv(outdir+r'\\Energy consumption_adjusted\\'+str(county_num)+'\\'+fname)
            ...
            for i in lst_chg:
                sc_crarr[i-1] = 10
            try:
                # os.mkdir(outdir + r"\\Energy consumption_v2h_1\\")
                
                os.mkdir(outdir + r"\\Energy consumption_v2h_1\\"+str(county_num))
            except:pass
            try:
                os.mkdir(outdir + r"\\Energy consumption_v2h_1\\"+str(county_num)+"\\SC_opp_"+trucknm)
            except:pass
            try:
                os.mkdir(outdir + r"\\Energy consumption_v2h_1\\"+str(county_num)+"\\FC_opp_"+trucknm)
            except:
                pass
            df2 = pd.DataFrame(sc_crarr,columns=['slowCR'])
                
            df2.to_csv(outdir + r"\\Energy consumption_v2h_1\\"+str(county_num)+"\\SC_opp_"+trucknm+'\\'+fname+'.csv')
