# only home charging is available for home v2h
import time
...
def Parkingrecog(num0, num1):
    basedir = r'/nfs/turbo/seas-parthtv/jiahuic/Ford_CC'
    ...
    for county_num in range(num0,num1):
        ...
        flst = os.listdir(outdir+r'//Energy consumption_adjusted_3//'+str(county_num)+'//')
        flst = [i for i in flst if '.csv' in i]
        flst = [i for i in flst if  i[0]==i[2]==i[4]=='0']
        for fname in flst:
            cari = int(fname[2])
            urbi = int(fname[0])
            trucknm = ['SUV','Truck'][cari]

            df0 = pd.read_csv(outdir+r'//Energy consumption_adjusted_3//'+str(county_num)+'//'+fname)
            ...
            # range
            # 0 car 1 van
            # car 2023 Tesla Model 3 RWD-Specifications 
            ...
            for i in lst_chg:
                sc_crarr[i-1] = 10
            try:
                os.mkdir(outdir + r"//Energy consumption_v2h_ol//"+str(county_num))
            except:
                pass
            try:
                os.mkdir(outdir + r"//Energy consumption_v2h_ol//"+str(county_num)+"//FC_opp_"+trucknm)
            except:
                pass
            try:
                os.mkdir(outdir + r"//Energy consumption_v2h_ol//"+str(county_num)+"//SC_opp_"+trucknm)
            except:
                pass
            df2 = pd.DataFrame(sc_crarr,columns=['slowCR'])
                
            df2.to_csv(outdir + r"//Energy consumption_v2h_ol//"+str(county_num)+"//SC_opp_"+trucknm+'//'+fname+'.csv')
if __name__ == '__main__':
    # import warnings
    # warnings.filterwarnings("ignore")
    Parkingrecog(rank*4,(rank+1)*4)
