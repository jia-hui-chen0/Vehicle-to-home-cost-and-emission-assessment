...
execute_unload "results.gdx" sc fc vSOC sd; '''
    ...
    ctydf = pd.read_csv(outputdir+r'//Scripts//2020_Gaz_counties_national_0728_tempreg.csv')
    uqlst = np.unique(ctydf['tempreg'])
    try:
        
        os.mkdir(outputdir+r'//Results_v2h//Results_'+str(year))
    ...
    for county_num in range(num0,num1):
        ...
        for fname_i in range(len(fnames)):
            ...
            for year in [years]:
                ...
                for i1 in range(1):
                    for i2 in range(1):
                        fname_evhhs = os.listdir(outputdir+r'//dischargecons_2//'+str(county_num)+'_'+str(i1)+'_'+str(i2))
                        # fname_evhhs = [i for i in fnames_1 if ['20','35','50','65','80'][fname_i]+ '_0_0_0_' in i]
                        
                        for fname_evhh in fname_evhhs:
                            sd_drarr = pd.read_csv(outputdir +r'//dischargecons_2//'+str(county_num)+'_'+str(i1)+'_'+str(i2)+'//'+fname_evhh)['CR'].to_numpy()
                            ...
                            for cyclei in range(ttncycle):
                                ...
                                sd_dr = db.add_parameter_dc("sd_dr",[h], "set to 0")
                            ...
                            try: 
                                ...
                                os.mkdir(outputdir+r'//Results_v2h//Results_'+str(year)+r'//')
                            except: pass
                            try: os.mkdir(outputdir+r'//Results_v2h//Results_'+str(year)+r'//'+str(county_num)+'_'+str(i1)+'_'+str(i2)+r'//')
                            except: pass
                            df_res.T.to_csv(outputdir+r'//Results_v2h//Results_'+str(year)+r'//'+str(county_num)+'_'+str(i1)+'_'+str(i2)+r'//'+fname_evhh)
