...
def soc_drop(num0, num1):
    ...
    for county_num in range(num0,num1): # county num is now temp reg
        ...
        def tempimpact(tem1):
            ...
            return coeT1
        for fname in [j for j in os.listdir(driving_profile_foldername) if j[0]==j[2]==j[4]=='0' and j[-6:-4]!='21'][0:50]:
            try:
                ...
                try: os.mkdir(outdir+r'\\Energy consumption_adjusted\\')
                except: pass                
                try: os.mkdir(outdir+r'\\Energy consumption_adjusted\\'+str(county_num))
                except: pass
                outputdir = outdir+r'\\Energy consumption_adjusted\\'+str(county_num)+'\\'
...
if __name__ == '__main__':
    ...
    for i in range(48):
        if i != 48:
            p = Process(target=soc_drop, args=([i*10,i*10+10]))
        else:
            p = Process(target=soc_drop,args=([i*10,478]))
        p.start()
        procs.append(p)
    for p in procs:
