    ...
    for county_num in range(num0,num1):
        ...
        for fname_i in range(len(fnames)):
            ...
            for year in [years]:
                ...
                for i1 in range(1):
                    for i2 in range(1):
                        fnames_1 = os.listdir(outputdir+r'\\ResStock\\dischargecons\\'+str(county_num)+'_'+str(i1)+'_'+str(i2))
                        fname_evhhs = [i for i in fnames_1 if '50'+ '_0_0_0_' in i]
                        fname_evhhs = [i for i in fname_evhhs if i[0:2]=='50']
                        
                        for fname_evhh in fname_evhhs:
                            sd_drarr = pd.read_csv(outputdir +r'\\ResStock\\dischargecons\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+'\\'+fname_evhh)['CR'].to_numpy()
                            ...
                            try: os.mkdir(outputdir+r'\\Result_v2h\\Results_'+str(year)+r'\\'+str(county_num)+r'\\')
                            except: pass
                            df_res.T.to_csv(outputdir+r'\\Result_v2h\\Results_'+str(year)+r'\\'+str(county_num)+r'\\'+fname)
