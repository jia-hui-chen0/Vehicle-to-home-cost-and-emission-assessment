...
execute_unload "results.gdx" sc fc vSOC sd; '''
    ...
    # foldername = [i[0:-4] for i in foldername]
    foldername = []
    minfnlst = os.listdir(r'C:\Users\jiahuic\University of Michigan Dropbox\Jiahui Chen\Ford_CC\Result_v2h\Results_80_Y60_2030_cons_nocarb\0_0_0\\')
    for county_num in range(num0,num1):
        fnames = os.listdir(outputdir+r'\\Energy consumption_adjusted_1_2_Y82_db\\'+str(county_num))
        fnames = [i for i in fnames if 'conflict' not in i]
        
        fnames = [i for i in fnames if i[:6]=='0_0_0_']
        fnames = [i for i in fnames if r'.csv' in i][:15]
        for i1 in range(1):
            for i2 in range(1):
                result_dir = outputdir+r'\\Result_v2h\\Results_80_Y82_'+str(years)+r'_nocarb\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+r'\\'
        ...
        i1 = 0
        i2 = 0
        try: os.mkdir(outputdir+r'\\Result_v2h\\Results_80_Y82_'+str(year)+r'_cons_nocarb\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+r'\\')
        except: pass
        # fnames = [i for i in fnames if i not in os.listdir(outputdir+r'\\Result_v2h\\Results_80_Y82_2040\\'+str(county_num)+'_'+str(0)+'_'+str(0)+r'\\')]
        existingfnlst = os.listdir(outputdir+r'\\Result_v2h\\Results_80_Y82_'+str(year)+r'_cons_nocarb\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+r'\\')
        if not all(fname[:-8]+'_50'+fname[-4:] in existingfnlst for fname in fnames):
            for fname_i in range(len(fnames)):
                costttl = 0

                fname = fnames[fname_i]
                fname_evhh = fname[:-8]+'_50'+fname[-4:]
                if fname_evhh in existingfnlst:
                    continue
                ...
                for year in [years]:
                    ...
                    for i1 in range(1):
                        for i2 in range(1):
                            sd_drarr_0 = pd.read_csv(outputdir +r'\\ResStock\\dischargecons_Y82_db\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+'\\'+fname_evhh)['CR'].to_numpy()
                            hours_per_day = 24
                            pm_start, pm_end = 17, 21
                            indices = np.array([day * hours_per_day + hour for day in range(365) for hour in range(pm_start, pm_end)])
                            # Cap values at 5 kW for only the selected hours
                            sd_drarr_0[indices] = np.minimum(sd_drarr_0[indices], 5)
                            indices_1 = [i for i in np.arange(8760) if i not in indices]
                            sd_drarr_0[indices_1] = 0
                            CR = dict(zip(hr,sd_drarr_0))
                            totalresults = [[],[],[],[]]
                            ...
                            try: 
                                # os.mkdir(outputdir+r'\\Results_opt_3\\')
                                os.mkdir(outputdir+r'\\Result_v2h\\Results_80_Y82_'+str(year)+r'_cons_nocarb\\')
                            except: pass
                            try: os.mkdir(outputdir+r'\\Result_v2h\\Results_80_Y82_'+str(year)+r'_cons_nocarb\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+r'\\')
                            except: pass
                            df_res.T.to_csv(outputdir+r'\\Result_v2h\\Results_80_Y82_'+str(year)+r'_cons_nocarb\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+r'\\'+fname_evhh)
...
if __name__ == '__main__':
    ...
    try:
        for year in ['2024','2030','2040']:
            os.listdir(rf'C:\Users\jiahuic\Dropbox (University of Michigan)\Ford_CC\Result_v2h\Results_80_Y82_{year}_cons_nocarb')
    except:
        pass
    a = 44
    b = 10
