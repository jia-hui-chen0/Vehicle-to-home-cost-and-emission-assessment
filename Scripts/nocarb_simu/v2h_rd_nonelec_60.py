    ...
    for county_num in range(num0,num1):
        ...
        for fname_i in range(len(fnames)):
            ...
            for year in [years]:
                ...
                for i1 in range(1):
                    for i2 in range(1):
                        sd_drarr = pd.read_csv(outputdir +r'\\ResStock\\dischargecons_oelec_Y60\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+'\\'+fname_evhh)['CR'].to_numpy()
                        ...
                        try: 
                            # os.mkdir(outputdir+r'\\Results_opt_3\\')
                            os.mkdir(outputdir+r'\\Result_v2h_no\\Results_'+str(year)+r'_60_nocarb\\')
                        except: pass
                        try: os.mkdir(outputdir+r'\\Result_v2h_no\\Results_'+str(year)+r'_60_nocarb\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+r'\\')
                        except: pass
                        df_res.T.to_csv(outputdir+r'\\Result_v2h_no\\Results_'+str(year)+r'_60_nocarb\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+r'\\'+fname_evhh)
if __name__ == '__main__':
    ...
    for yea in [2040]:
        ...
        for i in range(a):
            if i != a-1:
                p = Process(target=CC, args=([6+i*b,i*b+b,yea]))
            else:
                p = Process(target=CC,args=([6+i*b,432,yea]))
            p.start()
            procs.append(p)
        for p in procs:
            p.join() 