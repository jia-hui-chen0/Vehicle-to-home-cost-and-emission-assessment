    ...
    for county_num in range(num0,num1):
        ...
        for fname_i in range(len(fnames)):
            ...
            for year in [years]:
                ...
                for i1 in range(1):
                    for i2 in range(1):
                        sd_drarr = pd.read_csv(outputdir +r'\\ResStock\\dischargecons_noelec\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+'\\'+fname_evhh)['CR'].to_numpy()
                        ...
                        try: 
                            # os.mkdir(outputdir+r'\\Results_opt_3\\')
                            os.mkdir(outputdir+r'\\Result_v2h_no\\Results_'+str(year)+r'_nocarb\\')
                        except: pass
                        try: os.mkdir(outputdir+r'\\Result_v2h_no\\Results_'+str(year)+r'_nocarb\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+r'\\')
                        except: pass
                        df_res.T.to_csv(outputdir+r'\\Result_v2h_no\\Results_'+str(year)+r'_nocarb\\'+str(county_num)+'_'+str(i1)+'_'+str(i2)+r'\\'+fname_evhh)
