...
def UC(num0,num1):
    ...
    SOC = [60,153]
    # fnames = os.listdir(outputdir+r'\\Energy consumption_adjusted_1_2_Y82\\0')
    ...
    for county_num in range(num0,num1):
        for fname in fnames:
            ...
            if cari==0 and urbi ==0 and lcci==0:
                ...
                if results[2][1]!=60*0.3:
                    df_res = pd.DataFrame(results,index=['slowCR','fastCR','vSOC'],columns=[i for i in range(8760)])
                    try:os.mkdir(outputdir+r'\\Results\\Results_80_60_rev\\')
                    except:pass
                    try:
                        os.mkdir(outputdir+r'\\Results\\Results_80_60_rev\\'+str(county_num)+r'\\')
                    except:
                        pass
                    df_res.T.to_csv(outputdir+r'\\Results\\Results_80_60_rev\\'+str(county_num)+r'\\'+fname)
