...
import numpy as np
from blast1.models.degradation_model import BatteryDegradationModel
...
class NMC_Gr_50Ah_B2(BatteryDegradationModel):
    ...
    def update_rates(self, stressors):
        ...
        kcal1 = np.trapz(kcal, x=t_secs) / delta_t_secs
        if kcal1 == 0:
            print(TdegKN,UaN)
        kcyc1 = np.trapz(kcyc, x=t_secs) / delta_t_secs

        # Store rates
        rates = np.array([kcal1, kcyc1])
