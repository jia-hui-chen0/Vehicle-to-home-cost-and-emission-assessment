...
# num0,num1,year,cc_ip,urb,car,lcc=0,1,2024,0,0,0,0
    ...
    for county_numi in range(len(county_num_lst)):
        ...
        for building in ['_0_0']:
            fnames = os.listdir(outputdir+r'\\Result'+cc_hand+r'\\Results_'+'Y82_'+year+r'\\'+str(county_num)+building)
            ...
            if len(fnames) > 0:
                ...
                for fname in fnames:
                    res = pd.read_csv(outputdir+r'\\Result'+cc_hand+r'\\Results_'+'Y82_'+year+r'\\'+str(county_num)+building+'\\'+fname)
