...
# num0,num1,year,cc_ip,urb,car,lcc=0,1,2024,0,0,0,0
    ...
    for county_numi in range(len(county_num_lst)):
        ...
        yr = year
        for building in ['']:
            fnames = os.listdir(outputdir+r'\\Result'+cc_hand+r'\\Results_'+'80_82_rev'+r'\\'+str(county_num)+building)
            ...
            if len(fnames) > 0:
                ...
                for fname in fnames:
                    res = pd.read_csv(outputdir+r'\\Result'+cc_hand+r'\\Results_'+'80_82_rev'+r'\\'+str(county_num)+building+'\\'+fname)
    ...
    df2.to_csv(outputdir+r'\\CostEmissionAssessment_revision\csv_trb\\'
                ...
                +str(cc_ip)+'_'+str(urb)+'_'+str(car)+'_'+str(lcc)+'_rev.csv')
